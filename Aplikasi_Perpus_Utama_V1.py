...
class AplikasiPerpusTerintegrasi:
    ...
    def migrasi_excel_ke_sqlite(self):
        """
        Sistem pembersih otomatis jika data ter-append dua kali lipat saat migrasi awal.
        Membaca ulang dari folder arsip cadangan dengan mode 'replace' agar data kembali normal.
        """
        import datetime
        
        # Jalur folder arsip tempat file Excel Anda berada sekarang
        folder_arsip = os.path.join(DIR_PERPUS, "Arsip_Migrasi")
        
        # Cari file Excel di folder utama atau di folder arsip
        mapping_migrasi = [
            ("Data_Peminjaman_Buku.xlsx", "data_peminjaman_buku"),
            ("QR_ID_BUKU.xlsx", "qr_id_buku"),
            ("Data_Siswa_Perpus.xlsx", "data_siswa_perpus"),
            ("Data_Buku_Umum.xlsx", "data_buku_umum")
        ]
        
        conn = sqlite3.connect(DB_PATH)
        data_diperbaiki = False
        
        for name_file, nama_tabel in mapping_migrasi:
            # Tentukan target file (cek di folder luar dulu, kalau tidak ada cek di folder arsip)
            target_path = os.path.join(FOLDER_KERJA, name_file)
            if not os.path.exists(target_path) and os.path.exists(folder_arsip):
                # Cari file yang mengandung nama tersebut di folder arsip
                for f in os.listdir(folder_arsip):
                    if name_file in f:
                        target_path = os.path.join(folder_arsip, f)
                        break
            
            if os.path.exists(target_path):
                try:
                    # Baca data master asli dari Excel
                    if nama_tabel == "qr_id_buku":
                        df_lama = pd.read_excel(target_path, header=None, dtype=str)
                        df_lama.columns = ["kolom_0", "qr_code", "kolom_2", "label_buku", "jurusan"][:len(df_lama.columns)]
                    else:
                        df_lama = pd.read_excel(target_path, dtype=str)
                    ...
                    # Pembersihan data kembar bawaan Excel
                    ...
                    elif nama_tabel == "data_peminjaman_buku":
                        # Bersihkan jika ada baris duplikat log yang tidak sengaja ter-copy di excel
                        df_lama = df_lama.drop_duplicates(keep='first')
                    
                    # PENTING: Gunakan 'replace' untuk menimpa tumpukan data ganda sebelumnya!
                    df_lama.to_sql(nama_tabel, conn, if_exists="replace", index=False)
                    data_diperbaiki = True
                    print(f"✨ [CLEANUP] Data tabel '{nama_tabel}' berhasil dinormalisasi ke jumlah asli.")
                    
                except Exception as e:
                    print(f"❌ [CLEANUP ERROR] Gagal membersihkan {name_file}: {e}")
                    
        conn.commit()
        conn.close()
        
        # Jika data sudah dibersihkan dan dinormalisasi, berikan alert info sekali saja
        if data_diperbaiki:
            print("✅ Seluruh database SQLite telah dibersihkan dari duplikasi data ganda!")
    ...
    def proses_scan_pengembalian_buku(self, event=None):
        kode_buku = self.ent_buku_kembali.get().strip()
        self.ent_buku_kembali.delete(0, tk.END)
        if not kode_buku: return
        if any(b['Kode Buku'] == kode_buku for b in self.daftar_kembali_dijepit):
            messagebox.showwarning("Duplikasi", "Buku sudah ada dalam antrean.")
            return
            
        conn = sqlite3.connect(DB_PATH)
        df_log = pd.read_sql_query("SELECT * FROM data_peminjaman_buku WHERE kode_satuan_buku = ? AND UPPER(status) = 'DIPINJAM'", conn, params=(kode_buku,))
        conn.close()
        
        if not df_log.empty:
            row = df_log.iloc[-1]
            buku_kembali = {"id_pk": str(row['id']), "Kode Buku": kode_buku, "Nama Siswa": row['nama_siswa'].upper(), "Rombel": row['rombel'], "Status": "KEMBALI"}
            self.daftar_kembali_dijepit.append(buku_kembali)
            no_urut = len(self.daftar_kembali_dijepit)
            self.tree_kembali.insert("", tk.END, values=(no_urut, buku_kembali['Kode Buku'], buku_kembali['Nama Siswa'], buku_kembali['Rombel'], "VALIDASI ✓"))
            self.btn_simpan_kembali.config(state=tk.NORMAL)
        else:
            messagebox.showwarning("Tidak Valid", f"Buku {kode_buku} tidak terdeteksi berstatus 'DIPINJAM'.")

    def eksekusi_simpan_pengembalian(self):
        if messagebox.askyesno("Konfirmasi", f"Proses pengembalian untuk {len(self.daftar_kembali_dijepit)} buku ini?"):
            try:
                conn = sqlite3.connect(DB_PATH)
                cursor = conn.cursor()
                tgl_sekarang = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                for b in self.daftar_kembali_dijepit:
                    cursor.execute("UPDATE data_peminjaman_buku SET status = 'KEMBALI', tanggal_kembali = ? WHERE id = ?", (tgl_sekarang, b['id_pk']))
                conn.commit()
                conn.close()
                self.jalankan_auto_backup_silent()
                messagebox.showinfo("Sukses", "Buku berhasil dikembalikan di database!"); self.tampilkan_layaran_sirkulasi()
            except Exception as e: messagebox.showerror("Gagal", f"Gagal update status: {e}")
    ...
    def tampilkan_layar_rekap(self):
        ...
        # Proses Agregasi Paket via SQLite
        if not self.df_master_buku.empty:
            ...
            for _, r in self.df_master_buku.iterrows():
                ...
                judul_real = self.dapatkan_judul_buku(kode_satuan)
        ...
        # Proses Agregasi Fiksi Umum via SQLite
        dict_agregat_umum = {}
        if not self.df_rekap_log.empty:
            ...
            if not df_isbn_log.empty:
                ...
                for id_f in ids_fiksi_unik:
                    j_title = self.dapatkan_judul_buku(id_f)
                    dict_agregat_umum[j_title] = dict_agregat_umum.get(j_title, 0) + 1
                total_judul_umum = len(dict_agregat_umum)

        # Live Real-Time Transaction Counter
        if not self.df_rekap_log.empty:
            total_terpinjam = len(self.df_rekap_log[self.df_rekap_log['status'].str.upper() == 'DIPINJAM'])
            
        total_tersedia = total_eks_semua - total_terpinjam
        if total_tersedia < 0: total_tersedia = 0

        # --- GRAPHIC AREA CONTAINER METRIK KANBAN CARDS ---
        ...
        if not self.df_master_buku.empty:
            for _, r in self.df_master_buku.iterrows():
                j_p = self.dapatkan_judul_buku(str(r['label_buku']))
    ...
    def proses_tampilkan_rekap_rombel(self, event=None):
        ...
        for item in self.tree_rombel.get_children(): self.tree_rombel.delete(item)
        if self.df_rekap_log.empty: return
        df_filtered = self.df_rekap_log[(self.df_rekap_log['rombel'] == rombel_pilihan) & (self.df_rekap_log['status'].str.upper() == 'DIPINJAM')]
        if df_filtered.empty: self.btn_lihat_detil.config(state=tk.DISABLED); messagebox.showinfo("Clean", "Kelas ini bersih bebas tanggungan."); return
        
        df_agregat = df_filtered.groupby(['nisn', 'nama_siswa']).size().reset_index(name='Jumlah')
        for _, r in df_agregat.iterrows(): self.tree_rombel.insert("", tk.END, values=(r['nisn'], r['nama_siswa'], f"{r['Jumlah']} Buku"))
        self.btn_lihat_detil.config(state=tk.NORMAL)

    def pop_up_jendela_detil_buku(self):
        item_terpilled = self.tree_rombel.focus()
        if not item_terpilled: return
        nilai_baris = self.tree_rombel.item(item_terpilled, 'values'); nisn_target = nilai_baris[0]; nama_target = nilai_baris[1]
        df_buku_anak = self.df_rekap_log[(self.df_rekap_log['nisn'] == nisn_target) & (self.df_rekap_log['status'].str.upper() == 'DIPINJAM')]
        
        popup = tk.Toplevel(self.root); popup.title(f"Detil - {nama_target}"); popup.geometry("700x380"); popup.configure(bg="#f4f6f9"); popup.transient(self.root); popup.grab_set()
        tk.Label(popup, text=f"DAFTAR KODE & JUDUL BUKU YANG BELUM KEMBALI", font=("Arial", 11, "bold"), bg="#202124", fg="white", pady=8).pack(fill=tk.X)
        tk.Label(popup, text=f"Siswa: {nama_target}\nNISN : {nisn_target}", font=("Arial", 10, "bold"), bg="#f4f6f9", justify=tk.LEFT, anchor=tk.W).pack(fill=tk.X, padx=15, pady=10)
        tree_popup = ttk.Treeview(popup, columns=("No", "Kode", "Judul"), show="headings", height=8)
        tree_popup.heading("No", text="No"); tree_popup.heading("Kode", text="Kode Seri Buku"); tree_popup.heading("Judul", text="Judul Buku / Mata Pelajaran")
        tree_popup.column("No", width=40, anchor=tk.CENTER); tree_popup.column("Kode", width=160, anchor=tk.W); tree_popup.column("Judul", width=450, anchor=tk.W); tree_popup.pack(fill=tk.BOTH, expand=True, padx=15, pady=5)
        no = 1
        for _, r in df_buku_anak.iterrows(): 
            kode_buku = r['kode_satuan_buku']; judul_buku = self.dapatkan_judul_buku(kode_buku)
            tree_popup.insert("", tk.END, values=(no, kode_buku, judul_buku)); no += 1
        tk.Button(popup, text="Tutup Detil", font=("Arial", 10, "bold"), bg="#d93025", fg="white", padx=15, command=popup.destroy).pack(pady=10)
