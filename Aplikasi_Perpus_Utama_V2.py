...
class AplikasiPerpusTerintegrasi:
    ...
    def proses_scan_pengembalian_buku(self, event=None):
        ...
        # KUNCI PERBAIKAN: Gunakan 'SELECT rowid, *' agar pasti memiliki ID unik dari SQLite
        ...
        if not df_log.empty:
            row = df_log.iloc[-1] # Ambil transaksi aktif mutakhir
            
            # KUNCI AMAN: Ambil 'id' jika ada, jika tidak ada gunakan 'rowid'
            if 'id' in row and pd.notna(row['id']):
                id_db = str(row['id'])
            elif 'rowid' in row and pd.notna(row['rowid']):
                id_db = str(row['rowid'])
            else:
                id_db = str(row.name) # Fallback ke index Pandas
            ...
            # KONFIRMASI INTERAKTIF UNTUK PUSTAKAWAN
            pesan_konfirmasi = (
            ...
            # Tampilkan Pop-Up Konfirmasi Ramah Pustakawan
    ...
    def eksekusi_simpan_pengembalian(self):
        ...
        if messagebox.askyesno("Konfirmasi Pengembalian", f"Proses pengembalian resmi untuk {len(self.daftar_kembali_dijepit)} buku ini?"):
            try:
                ...
                # Update status berbasis rowid/id
