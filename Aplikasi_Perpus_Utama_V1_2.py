...
class AplikasiPerpusTerintegrasi:
    ...
    def proses_scan_pengembalian_buku(self, event=None):
        ...
        conn = sqlite3.connect(DB_PATH)
        # Query lebih fleksibel: Mencari kode buku yang statusnya mengandung kata 'PINJAM'
        ...
        if not df_log.empty:
            row = df_log.iloc[-1] # Ambil transaksi peminjaman paling terakhir
            id_db = str(row['id'])
            ...
            self.tree_kembali.insert("", tk.END, values=(no_urut, kode_buku, nama_siswa, rombel_siswa, "VALIDASI ✓"))
            
            # AKTIFKAN TOMBOL PROSES
            self.btn_simpan_kembali.config(state=tk.NORMAL, bg="#1a73e8", fg="white")
            self.log_ke_gui(self.txt_log if hasattr(self, 'txt_log') else None, f"Scan Pengembalian: {kode_buku} - {nama_siswa}")
        else:
            messagebox.showwarning("Tidak Ditemukan", f"Buku dengan kode '{kode_buku}' TIDAK TERDETEKSI sedang dipinjam di sistem.")
