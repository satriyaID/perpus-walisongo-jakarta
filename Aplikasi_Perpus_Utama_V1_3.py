...
class AplikasiPerpusTerintegrasi:
    ...
    def proses_tampilkan_rekap_rombel(self, event=None):
        ...
        # Normalisasi NISN ke String Murni (menghindari bug float/int)
        df_filtered['nisn_str'] = df_filtered['nisn'].astype(str).str.split('.').str[0].str.strip()
        ...
        # 3. FILTER LOGIS JURUSAN (Proteksi silang agar paket jurusan lain tidak ikut terhitung)
        def is_valid_buku_rombel(row):
            kode = str(row['kode_satuan_buku']).strip().upper()
            rombel = str(row['rombel']).strip().upper()
            
            # Jika siswa MPLB, jangan hitung buku paket AKL / TJKT
            if "MPLB" in rombel:
                if any(x in kode for x in ["TJKT", "TKJ", "AKL"]): return False
            # Jika siswa AKL, jangan hitung buku paket MPLB / TJKT
            elif "AKL" in rombel:
                if any(x in kode for x in ["MPLB", "TJKT", "TKJ"]): return False
            # Jika siswa TJKT / TKJ, jangan hitung buku paket MPLB / AKL
            elif "TJKT" in rombel or "TKJ" in rombel:
                if any(x in kode for x in ["MPLB", "AKL"]): return False
        ...
        df_clean = df_filtered.drop_duplicates(subset=['nisn_str', 'Judul_Buku'])
        
        # Simpan ke class variable untuk referensi
        self.df_rombel_aktif_clean = df_clean
        
        # 5. Hitung Jumlah Buku Bersih per Siswa
        df_agregat = df_clean.groupby(['nisn_str', 'nama_siswa'])['Judul_Buku'].count().reset_index(name='Jumlah')
        
        for _, r in df_agregat.iterrows(): 
            self.tree_rombel.insert("", tk.END, values=(r['nisn_str'], r['nama_siswa'], f"{r['Jumlah']} Buku"))
    ...
    def pop_up_jendela_detil_buku(self):
        ...
        nisn_siswa = str(item_data[0]).split('.')[0].strip()
        ...
        df_log_siswa['nisn_str'] = df_log_siswa['nisn'].astype(str).str.split('.').str[0].str.strip()
        
        df_detil = df_log_siswa[(df_log_siswa['nisn_str'] == nisn_siswa) & 
        ...
        if not df_detil.empty:
            ...
            # Filter silang jurusan agar tidak menampilkan buku hantu jurusan lain
            rombel_siswa = str(df_detil['rombel'].iloc[0]).upper()
            if "MPLB" in rombel_siswa:
                df_detil = df_detil[~df_detil['kode_satuan_buku'].astype(str).str.upper().str.contains("TJKT|TKJ|AKL", regex=True)]
            elif "AKL" in rombel_siswa:
                df_detil = df_detil[~df_detil['kode_satuan_buku'].astype(str).str.upper().str.contains("MPLB|TJKT|TKJ", regex=True)]
            elif "TJKT" in rombel_siswa or "TKJ" in rombel_siswa:
                df_detil = df_detil[~df_detil['kode_satuan_buku'].astype(str).str.upper().str.contains("MPLB|AKL", regex=True)]

            # Hilangkan duplikasi berdasarkan judul buku unik
        ...
        tk.Label(top, text=f"DAFTAR BUKU DIPINJAM: {nama_siswa} ({nisn_siswa})", font=("Arial", 11, "bold"), bg="#1a73e8", fg="white", pady=8).pack(fill=tk.X)
